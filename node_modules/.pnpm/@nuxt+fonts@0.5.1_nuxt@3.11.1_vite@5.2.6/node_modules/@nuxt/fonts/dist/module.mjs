...
import { withTrailingSlash, withLeadingSlash, hasProtocol, joinURL, withQuery, withoutLeadingSlash } from 'ufo';
...
const FontFamilyInjectionPlugin = (options) => createUnplugin(() => {
  ...
  async function transformCSS(code, id) {
    ...
    async function addFontFaceDeclaration(fontFamily, fallbackOptions) {
      ...
      const fontURL = result.fonts[0]?.src.find((s2) => "url" in s2)?.url;
      if (fontURL) {
        id = id.replace(/\?.*$/, "");
        const urls = options.fontMap.get(id) || /* @__PURE__ */ new Set();
...
const module = defineNuxtModule({
  ...
  async setup(options, nuxt) {
    ...
    nuxt.hook("build:manifest", (manifest) => {
      if (!options.experimental?.addPreloadLinks)
        return;
