...
const fromFile = (path) => fontkit__namespace.open(path).then(unpackMetricsFromFont);
const fromBlob = async (blob) => new Promise((resolve, reject) => {
  blobToBuffer__default.default(blob, (err, buffer) => {
    ...
    try {
      resolve(unpackMetricsFromFont(fontkit__namespace.create(buffer)));
    } catch (e) {
      reject(e);
    }
  });
...
const fromUrl = async (url) => {
  ...
  if (typeof window === "undefined") {
    ...
    return unpackMetricsFromFont(fontkit__namespace.create(Buffer.from(data)));
  }
